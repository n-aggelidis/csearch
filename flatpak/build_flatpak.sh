...
# Wechsle immer in den Ordner des Skripts (flatpak/)
...
# Lese die Version automatisch aus der main.py aus
VERSION=$(grep -E '^__version__' "../src/main.py" | cut -d '"' -f 2)
...
BUILD_DIR="build-dir"
REPO_DIR="repo"
OUTPUT_BUNDLE="CSearch-${VERSION}.flatpak"

echo -e "${GREEN}Starting Flatpak build process for version ${VERSION}...${NC}"
...
if ! flatpak-builder --repo="$REPO_DIR" --default-branch="$VERSION" --force-clean "$BUILD_DIR" "$MANIFEST"; then
...
if ! flatpak build-bundle "$REPO_DIR" "$OUTPUT_BUNDLE" "$APP_ID" "$VERSION"; then
